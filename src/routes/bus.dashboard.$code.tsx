...
function BusDashboardPage() {
  ...
  const liveBusesByDir = useMemo<Record<1 | 2, { busId: string; segmentIndex: number; segmentProgress: number; speedMetersPerMin: number }[]>>(() => {
    ...
    for (const dir of [1, 2] as const) {
      ...
      // SPEC: distancia entre paradas = polilínea routed (prioridad).
      // Fallback haversine SOLO si falta el dato routed de ese tramo.
    ...
    return out;
  }, [compareTestEnabled, stopsByDir, clock, liveCompareRaw, liveCompareQuery.dataUpdatedAt, scheduleEtaByDir, stopCoords, engine, code]);
